...
def choropleth_map(selected_year, selected_season, selected_measurement):
  
    # Fetch weather data for the selected year and measurement
    weather_data = fetch_weather_data(selected_measurement, selected_year, selected_season)
...
def create_layout(years_options, measurements_options, seasons_options):
    ...
    initial_map_figure = choropleth_map(initial_year,initial_season, initial_measurement)
    ...
    layout = html.Div([
        ...
        html.Div([
            dcc.Dropdown(
                ...
                value=initial_year, 
                placeholder="Select a year",  # Placeholder text for the year dropdown
                clearable=True, 
                searchable=False,  # Disables search functionality
                style={'width': '150px', 'marginRight': '20px'}  # Set a fixed width for the dropdown and add right margin
            ),
             dcc.Dropdown(
                id='season-dropdown',
                options=seasons_options,
                value=initial_season,
                placeholder="Select a season",  # Placeholder text for the season dropdown
                clearable=True,
                searchable=False,
                style={'width': '150px', 'marginRight': '20px'}  # Set a fixed width for the dropdown and add right margin
            ),
            html.Div([
                ...
                dcc.Graph(id='choropleth-map', figure=initial_map_figure, style={'width': '100%'})  # Set width to 100% for the map
            ], style={'flex': '1'})  # Set the div containing radio items and map to occupy remaining space
        ], style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '20px'})  # Align items horizontally and center vertically
    ])
